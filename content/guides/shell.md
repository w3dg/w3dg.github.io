...
draft = false
title = "The command line is not scary, its powerful."
summary = "Most people when starting out in their tech career tend to overlook the possibilities that command line offers. Here is a guide from me on the what and why you should know to be decent at the command line."
tags = ['Guide', 'CLI', 'Unix', 'Bash', 'Shell Scripting']

+++

Most people when starting out in their tech career tend to overlook the possibilities that command line offers. Here is a guide from me on the what and why you should know to be decent at the command line.
...
With that preamble, I am writing this in the hope to help others to get started with the shell. I do not aim to go into a comprehensive detail but I will link all the resources that helped me or that I deem useful to beginners and experienced alike.
...
Long gone are the days that there is a single computer that everyone else connects to from their "terminal". Instead those terms are somewhat obsolete in their meaning in our everyday usage now.

Being old does not mean its not useful. Instead it's the most useful thing on a computer. Being comfortable at the command line opens up a new way of thinking. You understand the inner workings, the basics of the interface you are presented with and appreciate its simplicity for the power it bestows upon you.
...
If you are thinking its a simple concept, yes it is. But this simplicity is power. All the tools which are specialized to be used for one task can now be connected to each other to solve a larger task at hand.
...
- `awk` - Awk is like a programming language in itself and is good to work with columnar data or data that is semi structured in a columnar way. Many commands above like `grep`, `sed` , `cut`, `head` , `tail` etc. can be replaced with `awk`.
...
Now look again at the pipeline[^1] at the beginning of the article, can you make sense of at-least a vague idea of what it is trying to do?
...
You would probably want to learn a bit of Bash scripting. It can be as simple as a loop with a counter or a loop over the files in the directory to execute some command on each of them.

### Conditionals

`if` - tests for some condition. There are a few different ways to test a condition. There is the  `test` command and the `[` binary. These two are the same program with the `[` binary requiring the additional closing `]` as its last argument. There also is the , builtin `[[` to bash. Almost all of these accept the same kind of syntax. 

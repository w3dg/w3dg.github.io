...
- To assure reliability and availability of the service even when one server goes down

Many types of load balancing algorithms are present. Some can be static or dynamic.

Static load balancers usually have a *master* server that the request is hit and then it forwards the request to other servers.
...
Dynamic load balancers are more complex and can redirect traffic based on the current load of the servers. There needs to be some monitoring and chit-chat among the servers and the master to ensure that the load is distributed evenly.

## GeoDNS Load Balancing

Normally when I ping Google, I get a response somewhere close to me, like Chennai or Mumbai. In this particular instance, I got a response from Chennai it seems like. Here's the IP address I got:
